...
module.exports.verifyAccessToken = function verifyAccessToken(
  ...
  if (
    ...
    (event.queryStringParameters &&
      event.queryStringParameters["newAuthorizer"] === "MSAL")
  ) {
    // use MSAL to verify the token
    const decoded = jsonWebToken.decode(accessToken);
    if (!decoded || process.env.AAD_APPLICATION_ID !== decoded.appid) {
      console.error(
        "Decoded MSAL token is invalid: " + JSON.stringify(decoded)
      );
    ...
    (event.headers && event.headers["new-authorizer"] === "MSAL-CN") ||
    (event.queryStringParameters &&
      event.queryStringParameters["newAuthorizer"] === "MSAL-CN")
  ) {
    // use MSAL-CN to verify the token
    const decoded = jsonWebToken.decode(accessToken);
    if (!decoded || process.env.AAD_CN_APPLICATION_ID !== decoded.appid) {
      console.error(
        "Decoded MSAL-CN token is invalid: " + JSON.stringify(decoded)
      );
      return context.fail("Unauthorized");
    }
